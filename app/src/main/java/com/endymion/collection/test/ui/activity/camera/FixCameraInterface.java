 ...
 */
public class FixCameraInterface implements Camera.PreviewCallback {

    private static final String TAG = "CJT";

    private volatile static FixCameraInterface mCameraInterface;
    ...
    //视频质量
    private int mediaQuality = FixJCameraView.MEDIA_QUALITY_MIDDLE;
    private SensorManager sm = null;

    //获取CameraInterface单例
    public static synchronized FixCameraInterface getInstance() {
        if (mCameraInterface == null)
            synchronized (CameraInterface.class) {
                if (mCameraInterface == null)
                    mCameraInterface = new FixCameraInterface();
            }
     ...
     * open Camera
     */
    void doOpenCamera(CameraInterface.CameraOpenOverCallback callback) {
...
//                destroyCameraInterface();
    ...
    public void takePicture(final CameraInterface.TakePictureCallback callback) {
        if (mCamera == null) {
            return;
        }
        switch (cameraAngle) {
...
//
    ...
    //启动录像
    public void startRecord(Surface surface, float screenProp, FixCameraInterface.ErrorCallback callback) {
        mCamera.setPreviewCallback(null);
